...
ap.add_argument("--months", default="2-12")
ap.add_argument("--strategies", default="noupdate,noupdate+QM,noupdate+R,L0L1_5,L0L1_5+R,L2,L2+R")
...
RES = os.path.join(ROOT, "results", "tls", args.tag)
...
for month in months:
    mdays = month_days(days, month)
    if not mdays: continue
    Dm = prep(load_days(NAME, mdays, ppi_len=args.ppi_len))
    P_(f"== month {month}: {len(Dm['y'])} flows known {Dm['is_known'].sum()} utest {Dm['is_utest'].sum()} ucal {Dm['is_ucal'].sum()}")
