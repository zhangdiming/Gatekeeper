...
def gate_mask(tab, D):
    uniq, inv = np.unique(D["ip"], return_inverse=True); U = np.ones((len(uniq), K), bool); known_ip = np.zeros(len(uniq), bool); udom = np.full(len(uniq), -1, np.int64)
    for j, ip in enumerate(uniq):
        if ip in tab.ip: known_ip[j] = True; U[j] = False; U[j, list(tab.ip[ip].keys())] = True; udom[j] = tab.ip[ip].most_common(1)[0][0]
    mask = U[inv]; new = ~known_ip[inv]; dom = udom[inv]
    ua, ainv = np.unique(D["asn"], return_inverse=True); A = np.ones((len(ua), K), bool); ak = np.zeros(len(ua), bool); adom = np.full(len(ua), -1, np.int64)
    for j, a in enumerate(ua):
        if a in tab.asn: ak[j] = True; A[j] = False; A[j, list(tab.asn_classes(a))] = True; adom[j] = tab.asn[a].most_common(1)[0][0]
    sel = new & ak[ainv]; mask[sel] = A[ainv[sel]]; dom[sel] = adom[ainv[sel]]
    return mask, dom
...
for month in range(2, 13):
    ...
    if not md: continue
    Dm = load(md); m = Dm["y"] >= 0; y = Dm["y"]; mask, dom = gate_mask(tab, Dm)
    ...
    predm = np.where(noallow, np.where(dom[sel] >= 0, dom[sel], pred), predm)
    r.update(knn_global=float((pred == y[sel]).mean()), knn_mask=float((predm == y[sel]).mean()), knn_n=int(len(sel)), knn_no_allowed_neighbor_k10=float(noallow.mean()), knn_s=time.time() - t2)
    rows.append(r); print(r, flush=True)
    pd.DataFrame(rows).to_csv(os.path.join(RES, "extra_knn_seed0.csv"), index=False)
json.dump(dict(knn_train=len(kidx_)), open(os.path.join(RES, "extra_knn_meta.json"), "w"))
